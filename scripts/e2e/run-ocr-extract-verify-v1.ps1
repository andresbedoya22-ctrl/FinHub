...
# =========================
# PHASE7 v1: OCR -> IA Extraction -> (fallback activeringscode) -> Verify
# =========================

# --- 6) Run extraction (IA) ---
Write-Host "`n[6] POST /api/documents/$docId/extraction (IA)"
try {
  $aiRes = Invoke-RestMethod -Method POST -Uri "$baseUrl/api/documents/$docId/extraction" -WebSession $session `
    -Headers @{ "content-type"="application/json" } `
    -Body "{}"
} catch {
  $body = Read-HttpErrorBody $_
  if ($body) { throw ("POST /api/documents/$docId/extraction failed. Body: " + $body) }
  throw ("POST /api/documents/$docId/extraction failed. Error: " + $_.Exception.Message)
}
Assert-Ok ($aiRes.ok -eq $true) ("Extraction (IA) failed: " + (Json $aiRes))
Write-Host "Extraction IA ok."

# --- 7) Get latest extraction ---
Write-Host "`n[7] GET /api/documents/$docId/extraction"
...
$ex = $exRes.extraction
Assert-Ok ($ex) "No extraction returned after IA."
Write-Host "ExtractionId: $($ex.id); needs_review=$($ex.needs_review)"

# --- 8) Patch only if activeringscode missing (min viable for verify) ---
$act = ""
try {
  # PowerShell no soporta ?? en todas las versiones, así que lo hacemos defensivo
  if ($null -ne $ex -and $null -ne $ex.fields -and $null -ne $ex.fields.activeringscode) { $act = $ex.fields.activeringscode.ToString() }
} catch { $act = "" }

if (-not $act -or $act.Trim().Length -lt 6) {
  Write-Host "`n[8] PATCH /api/documents/$docId/extraction (activeringscode fallback)"
  $patch = @{
    fields = @{
      activeringscode = "ABC12345"
      extra = @{ source = "e2e"; stamp = $stamp; note = "fallback activeringscode for verify" }
    }
    needsReview = $true
    confidence = $null
  }
  ...
  Write-Host "Patch fallback OK. extractionId=$($patchRes.extractionId)"
} else {
  Write-Host "`n[8] Patch not needed (activeringscode present): $act"
}

# --- 9) Verify ---
Write-Host "`n[9] POST /api/documents/$docId/verify"
...
# --- 10) Check document status ---
Write-Host "`n[10] GET /api/documents/$docId"
...
Write-Host "`nE2E Phase7 (OCR + IA extraction + verify) OK."
