...
Assert-Ok ($docRes.ok -eq $true) ("Create document failed: " + ($docRes.error | Out-String))
$docId = $docRes.doc.id
...
Assert-Ok ($ocrRes.ok -eq $true) ("OCR failed: " + ($ocrRes.error | Out-String))
Write-Host "OCR ok. provider/run/extraction returned."
...
Assert-Ok ($exRes.ok -eq $true) ("Get extraction failed: " + ($exRes.error | Out-String))
$ex = $exRes.extraction
Assert-Ok ($ex) "No extraction returned."
$exId = $ex.id
Write-Host "ExtractionId: $exId; needs_review=$($ex.needs_review)"

# --- 7) Patch extraction fields (mínimo viable) ---
Write-Host "`n[7] PATCH /api/documents/$docId/extraction (set activeringscode)"
$patch = @{
  fields = @{
...
Assert-Ok ($patchRes.ok -eq $true) ("Patch extraction failed: " + ($patchRes.error | Out-String))
...
Assert-Ok ($verRes.ok -eq $true) ("Verify failed: " + ($verRes.error | Out-String))
Write-Host "Verify OK. extractionId=$($verRes.extractionId)"

# --- 9) Check document status ---
Write-Host "`n[9] GET /api/documents/$docId (check status)"
$docGet = Invoke-RestMethod -Method GET -Uri "$baseUrl/api/documents/$docId" -WebSession $session
Assert-Ok ($docGet.ok -eq $true) ("Get document failed: " + ($docGet.error | Out-String))
