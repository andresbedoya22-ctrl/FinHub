...
export function RegisterClient() {
  ...
  const [role, setRole] = useState<"employee" | "zzp" | "both" | "">("");
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      // Pasamos el role también en el payload al register local si soporta perfiles
  ...
  const errMsg = errorText(errorCode) || (errorCode === 'missing_role' ? "Please select your employment type." : null);
  ...
  return (
    <div className="flex min-h-screen bg-[#0D1B2A] text-gray-100">
          ...
          <div className="space-y-6">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="bg-[#0D1B2A] px-2 text-gray-500">Or register with email</span>
              </div>
            </div>
            ...
            <form className="space-y-5" onSubmit={onSubmit}>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300 block mb-2">Employment Type in NL *</label>
                <div className="grid grid-cols-3 gap-2">
                  {['employee', 'zzp', 'both'].map((r) => (
                    <button
                      key={r}
                      type="button"
                      onClick={() => setRole(r as "employee" | "zzp" | "both")}
                      className={`py-2 px-1 text-sm font-medium rounded-lg border transition-all ${role === r
                        ? 'border-[#4CAF50] bg-[#4CAF50]/10 text-white'
                        : 'border-gray-700 bg-white/5 text-gray-400 hover:border-gray-500 hover:text-gray-300'
                        }`}
                    >
                      {r === 'employee' ? 'Employee' : r === 'zzp' ? 'Autónomo/ZZP' : 'Both'}
                    </button>
                  ))}
                </div>
              </div>
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-12 bg-gradient-to-bl from-[#0D1B2A] to-[#1a3652] border-l border-white/5">
        ...
        <div className="space-y-6">
          <h1 className="text-4xl font-bold leading-tight text-right">
            Join the only financial hub tailored for migrants
          </h1>
          <p className="text-lg text-gray-400 text-right ms-auto max-w-md">
            Stop worrying about complex Dutch letters and isolated bank apps. Bring everything together.
          </p>

          <div className="pt-8 grid gap-4 place-items-end">
            <div className="flex items-center gap-3 text-sm text-gray-300">
              Translations available inside
