...
export default function FinancesDashboardClient() {
  ...
  const useMock = process.env.NEXT_PUBLIC_FH_USE_MOCK_FINANCES === "1";
  ...
  const effectiveCategories = useMemo(() => {
    ...
    if (useMock || ledgerErr) return mock.categories;
    return [];
  }, [categories, useMock, ledgerErr, mock.categories]);

  const transactions: FinanceTransaction[] = useMemo(() => {
    if (persistedTx.length) return persistedTx;
    if (useMock || ledgerErr) return mock.transactions;
    return [];
  }, [persistedTx, useMock, ledgerErr, mock.transactions]);
  ...
  const paletteActions = [
    ...
    ...MODULES.map((m) => ({ id: `case_${m.type}`, label: `Iniciar: ${m.title}`, hint: m.subtitle, keywords: [m.pill, String(m.type)], run: () => void launch(m.type) })),
  ];

  const showLoading = bootstrapLoading || ledgerLoading;
  ...
  return (
    <Screen className="space-y-6">
      ...
      <Card className="space-y-3">
        ...
        {showLoading ? <div className="text-sm text-fh-muted">Cargando datos</div> : null}

        {bootstrapErr ? (
          <div className="rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-3 py-2 text-sm">
            No se pudo cargar persistencia del plan (usando default): {bootstrapErr}
          </div>
        ) : null}

        {ledgerErr ? (
          <div className="rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-3 py-2 text-sm">
            Ledger no disponible (fallback UI): {ledgerErr}
          </div>
        ) : null}

        {useMock ? (
          <div className="rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-3 py-2 text-sm">
            Modo demo activo (NEXT_PUBLIC_FH_USE_MOCK_FINANCES=1).
          </div>
        ) : null}
          ...
          <div className="absolute left-1/2 top-24 w-[92vw] max-w-[720px] -translate-x-1/2 rounded-2xl border border-fh-border bg-fh-surface p-4 shadow-xl">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <div>
                ...
                <input className="mt-1 w-full rounded-xl border border-fh-border bg-fh-surface px-3 py-2 text-sm" value={cNote} onChange={(e) => setCNote(e.target.value)} placeholder="Opcional" />
              </div>
            </div>

            <div className="mt-4 flex justify-end gap-2">
              <button className="rounded-xl border border-fh-border bg-fh-surface px-4 py-2 text-sm hover:bg-fh-surface-2" onClick={() => setCreateOpen(false)}>
                Cancelar
              </button>
              <button
                ...
                onClick={async () => {
                  ...
                  await createTx({
                    ...
                    amountCents: Number.isFinite(amt) ? amt : 0,
